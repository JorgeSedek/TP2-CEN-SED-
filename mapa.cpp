...
void Mapa::recolectar_recursos(Material *vector_materiales, int tipos_materiales, Ubicacion *vector_ubicaciones, int edificios_construidos){
	for (int i = 0; i < filas ; i++) {
		for (int j = 0; j < columnas ; j++){
			if (matriz[i][j]->obtener_tipo_casillero() == CONSTRUIBLE && matriz[i][j]->obtener_nombre_contenido() != ""){
				string nombre_edificio = encontrar_edificio(vector_ubicaciones, i, j, edificios_construidos);

				for (int i = 0; i < tipos_materiales; i++){
					vector_materiales[i].producir_material(nombre_edificio);
...
string Mapa::encontrar_edificio(Ubicacion *vector_ubicaciones,int fila, int columna, int edificios_construidos){

	int k = 0;
	while((vector_ubicaciones[k].obtener_fila() != fila && vector_ubicaciones[k].obtener_columna() != columna) && k < edificios_construidos ){
		k++;
	}
	return vector_ubicaciones[k].obtener_nombre();
